...
std::vector<float> ofxCvFaceRec::getSimilarities(ofxCvGrayscaleImage img)
{
    ...
    if(!trained) {
        if( !loadTrainingData( &trainPersonNumMat ) ) return distList;
    };
    ...
    return findDistance(projectedTestFace);
...
std::vector<float> ofxCvFaceRec::findDistance(float * projectedTestFace) {
